...
// Include everything
define('KMD_MUST_LOAD_WP',  '1');
include (dirname(__FILE__) . '/kmd-include-all.php');

// Cpanel-scheduled cron job call
if (@$_REQUEST['hardcron']=='1')
  KMD_cron_job_worker (true);
else {
    add_action( 'woocommerce_after_register_post_type', 'KMD_cron_job_worker', 10);
}
//===========================================================================
// '$hardcron' == true if job is ran by Cpanel's cron job.

function KMD_cron_job_worker ($hardcron=false)
{
  global $wpdb;
  ...
  $confirmations_required = $kmd_settings['confs_num'];
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta    = KMD_unserialize_address_meta (@$row_for_balance_check['address_meta']);
		  ...
		  // Retrieve current balance at address considering required confirmations number and api_timemout value.
			$address_request_array['address'] = $row_for_balance_check['kmd_address'];
		  ...
		  $balance_info_array = KMD__getreceivedbyaddress_info ($address_request_array, $kmd_settings);
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          KMD__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['kmd_address']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            KMD__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['kmd_address']}' (KMD '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
        else
        {
        ...
        //	Scan '$address_meta['orders']' for first UNPAID order that is exactly matching amount at address.
	        ...
	        KMD__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['kmd_address']}' (KMD '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        KMD__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
          ...
          //
	        $query =
	          "UPDATE `$kmd_addresses_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        KMD__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");
...
// 	        if (!$hardcron && $ran_cycles >= $kmd_settings['soft_cron_max_loops_per_run'])
// 	        {
...
// 	        }
		  ...
		  {
		    KMD__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['kmd_address']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  //-----------------------------------------------------
  // Pre-generate new komodo address for electron cash wallet
  ...
  if ($hardcron)
  {
    ...
    if ($electrum_mpk && @$kmd_settings['service_provider'] == 'electrum_wallet')
    {
      ...
      $query =
        "SELECT COUNT(*) as `total_unused_addresses` FROM `$kmd_addresses_table_name`
      ...
      if ($total_unused_addresses < $kmd_settings['max_unused_addresses_buffer'])
      {
          KMD__generate_new_address_for_electrum_wallet ($kmd_settings, $electrum_mpk);
      }
    }
  }
  //-----------------------------------------------------
